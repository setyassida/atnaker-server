  ...
  class Barcode {
    ...
    static private function _draw($call, $res, $color, $x, $y, $angle, $type, $datas, $width, $height){
	  ...
	  if (gettype($res) != "resource") {
		$res->SetFont('Arial','',8);
		$res->Text($x-25, $y+$height/2+8, $code);
		$res->SetFont('Times','',12);
	  } else {
		$font = $_SERVER['DOCUMENT_ROOT']."/doc/pdf/lib/arial.ttf";
		imagettftext($res, 8, 0, $x-25, $y+$height/2+12, $color, $font, $code);
	  }
      $result['hri'] = $hri;
      return $result;
    }
